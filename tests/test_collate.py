...
class test_Collate(ut.TestCase):
    def test_collate_labels(self):
        ...
        self.assertEqual(collated_labels["label1"].shape, torch.Size([num_labels, 1]))  # , 1
        self.assertEqual(collated_labels["label2"].shape, torch.Size([num_labels * 5, 1]))  # , 5
        self.assertEqual(collated_labels["label3"].shape, torch.Size([num_labels * 5, 2]))  # , 5, 2
        ...
        np.testing.assert_array_equal(collated_labels["label1"].numpy(), label1_true.numpy())
        np.testing.assert_array_equal(
            collated_labels["label2"].numpy(), label2_true.flatten(0, 1).unsqueeze(1).numpy()
        )
        np.testing.assert_array_equal(collated_labels["label3"].numpy(), label3_true.flatten(0, 1).numpy())
        ...
        self.assertEqual(collated_labels["label1"].shape, torch.Size([num_labels, 1]))
        self.assertEqual(collated_labels["label2"].shape, torch.Size([num_labels * 5, 1]))  # , 5
        self.assertEqual(collated_labels["label3"].shape, torch.Size([num_labels * 5, 2]))  # , 5, 2
