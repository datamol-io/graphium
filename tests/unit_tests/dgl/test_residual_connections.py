        ...
        for skip_steps in [1, 2, 3]:
            ...
            for ii in range(num_loops):
                ...
                if ((ii % skip_steps) == 0) and (ii > 0):
                    h_expected = (torch.sum(torch.stack(h_original[0:ii+1:skip_steps], dim=0), dim=0)).numpy()
                ...
                np.testing.assert_array_equal(h.numpy(), h_expected, err_msg=f"Error at: skip_steps={skip_steps}, ii={ii}")
                np.testing.assert_array_equal(h_prev.numpy(), h_expected_prev, err_msg=f"Error at: skip_steps={skip_steps}, ii={ii}")
...
class test_ResidualConnectionWeighted(ut.TestCase):
    ...
    def test_forward_weighted(self):
        ...
        for skip_steps in [1, 2, 3]:
            ...
            rc = ResidualConnectionWeighted(skip_steps=skip_steps, full_dims=full_dims, 
            ...
            for ii in range(num_loops):
                ...
                np.testing.assert_array_equal(h.detach().numpy(), h_expected, err_msg=f"Error at: skip_steps={skip_steps}, ii={ii}")
                np.testing.assert_array_equal(h_prev.detach().numpy(), h_expected_prev, err_msg=f"Error at: skip_steps={skip_steps}, ii={ii}")
    ...
    def test_forward_concat(self):
        ...
        for skip_steps in [1, 2, 3]:
            ...
            for ii in range(num_loops):
                ...
                if ((ii % skip_steps) == 0) and (ii > 0):
                    h_expected = (torch.cat(h_original[ii-skip_steps:ii+1:skip_steps][::-1], dim=-1)).numpy()
