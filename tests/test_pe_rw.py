...
class test_pe_spectral(ut.TestCase):
    ...
    def test_caching_and_outputs(self):
        ...
        pe1, _, cache = compute_rwse(adj.astype(np.float32), ksteps1, num_nodes, cache,
                                             pos_type="rw_transition_probs")

        pe2, _, cache = compute_rwse(adj.astype(np.float32), ksteps2, num_nodes, cache,
                                             pos_type="rw_return_probs")

        pe3, _, cache = compute_rwse(adj.astype(np.float32), ksteps3, num_nodes, cache,
                                             pos_type="rw_return_probs")
        ...
        self.assertGreaterEqual(pe1.min(), 0.)
        self.assertLessEqual(pe1.max(), 1.)

        self.assertGreaterEqual(pe2.min(), 0.)
        self.assertLessEqual(pe2.max(), 1.)

        self.assertGreaterEqual(pe3.min(), 0.)
        self.assertLessEqual(pe3.max(), 1.)
