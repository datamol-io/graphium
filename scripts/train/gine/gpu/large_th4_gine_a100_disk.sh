...
#SBATCH --job-name=large_th4_gine_a100_disk

## Files for logs: here we redirect stoout and sterr to the same file
#SBATCH --output=outputs/large_th4_gine_a100_disk.out
#SBATCH --error=outputs/error_large_th4_gine_a100_disk.out
...
micromamba run -n graphium -c graphium-train architecture=largemix tasks=largemix training=largemix \
    ...
    trainer.model_checkpoint.dirpath="model_checkpoints/large-dataset/th4/gine/gpu/" \
