...
def any_smiles_iterable(
    smiles, sanitize=False, check_mass=True, min_count=1, mute_errors=True
):
    r"""
...
def is_smiles_iterable(
    smiles, sanitize=False, check_mass=True, mute_errors=True, verbose=0
):
    r"""
    ...
    with SuppressRdkit(mute_errors=mute_errors):
        ...
        if isinstance(smiles, str) or not is_iterable(smiles):
            is_smiles_array = is_smiles(
                smiles, sanitize=sanitize, check_mass=check_mass, mute_errors=False
            )
        ...
        # If it is an iterable, return a boolean array iterable
            ...
            for index in np.ndindex(smiles.shape):
                ...
                if (verbose >= 3) and (not is_smiles_bool):
                    print(
                        'Bad SMILES "{}" detected at index {}'.format(
                            smiles[index], index
                        )
                    )
