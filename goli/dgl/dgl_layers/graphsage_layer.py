...
class GraphSageLayer(BaseDGLLayer):
    ...
    def __init__(self, in_dim, out_dim, activation, dropout,
        ...
        if dgl_builtin == False:
            self.nodeapply = NodeApply(in_dim, out_dim, activation, dropout,
                                   bias=bias)
            if aggregator_type == "maxpool":
                self.aggregator = MaxPoolAggregator(in_dim, in_dim,
                                                    activation, bias)
            ...
            self.sageconv = SAGEConv(in_dim, out_dim, aggregator_type,
