...
class PNATower(nn.Module):
    def __init__(
        ...
        self.pretrans = MLP(
            in_dim=2 * in_dim + (edge_dim if edge_features else 0),
            hidden_size=in_dim,
            out_dim=in_dim,
        ...
        self.posttrans = MLP(
            in_dim=(len(aggregators) * len(scalers) + 1) * in_dim,
            hidden_size=out_dim,
            out_dim=out_dim,
...
class PNALayer(BaseDGLLayer):
    def __init__(
        ...
        assert (
            not divide_input
        ) or in_dim % towers == 0, "if divide_input is set the number of towers has to divide in_dim"
    ...
    def forward(self, g, h, e=None, snorm_n=None):
        ...
        if self.divide_input:
            h_cat = torch.cat(
                [
                    tower(g, h[:, n_tower * self.input_tower : (n_tower + 1) * self.input_tower], e, snorm_n)
