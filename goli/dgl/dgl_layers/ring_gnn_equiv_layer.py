...
class RingGNNEquivLayer(nn.Module):
    def __init__(
        ...
        coeffs_values = lambda i, j, k: torch.randn([i, j, k]) * torch.sqrt(2.0 / (i + j).float())
        self.diag_bias_list = nn.ParameterList([])

        for i in range(radius):
            for j in range(i + 1):
                self.diag_bias_list.append(nn.Parameter(torch.zeros(1, out_dim, 1, 1)))

        self.all_bias = nn.Parameter(torch.zeros(1, out_dim, 1, 1))
        self.coeffs_list = nn.ParameterList([])

        for i in range(radius):
            for j in range(i + 1):
                self.coeffs_list.append(nn.Parameter(coeffs_values(in_dim, out_dim, basis_dimension)))

        self.switch = nn.ParameterList(
            [nn.Parameter(torch.FloatTensor([1])), nn.Parameter(torch.FloatTensor([k2_init]))]
        )
        ...
        if self.layer_norm:
            self.ln_x = LayerNorm(out_dim.item())

        if self.residual:
            self.res_x = nn.Linear(in_dim.item(), out_dim.item())
    ...
    def forward(self, inputs):
        ...
        for i in range(self.radius):
            for j in range(i + 1):
                ...
                mat_diag_bias = (
                    torch.eye(inputs.size()[3]).unsqueeze(0).unsqueeze(0).to(self.device)
                    * self.diag_bias_list[i * (i + 1) // 2 + j]
                )
        ...
        if self.layer_norm:
            ...
            # output = self.bn_x(output.reshape(m*m, self.out_dim.item())) # batch normalization
            output = self.ln_x(output)  # layer normalization

            # Returning output back to original shape
            output = output.reshape(m, m, self.out_dim.item())
