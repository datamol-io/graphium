...
class IntermittentPoolingLayer(nn.Module):
    ...
    """
    def __init__(self, in_dim, num_layers=2, pooling='sum', activation='relu', 
                    last_activation=None, dropout=0., batch_norm=False, bias=True, 
        ...
        self.mlp = MLP(
                in_dim=in_dim, hidden_size=in_dim, out_dim=in_dim, layers=num_layers, 
                mid_activation=activation, last_activation=last_activation,
                dropout=dropout, mid_batch_norm=batch_norm, last_batch_norm=batch_norm, device=device)
    ...
    def forward(self, g, h):
        ...
        if len(self.pool_layer) > 0:
            ...
            for ii, num_nodes in enumerate(g.batch_num_nodes):
                ...
                pooled_expanded = pooled[ii:ii+1].expand(this_h.shape[:-1] + pooled[ii:ii+1].shape[-1:])
