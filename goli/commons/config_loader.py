...
def config_load_siamese_gnn(cfg_model, cfg_gnns, in_dim, out_dim, device, dtype):
    ...
    gnn_kwargs = dict(in_dim=in_dim,
                      out_dim=cfg_model['dist_vector_size'],
                      **cfg_model['gnn_kwargs'], **gnn_layer_kwargs)
    ...
    gnn = SiameseGraphNetwork(gnn_kwargs=gnn_kwargs,
...
def config_load_gnn(cfg_model, cfg_gnns, in_dim, out_dim, device, dtype):
    ...
    lnn_kwargs = dict(in_dim=gnn_kwargs['out_dim'],
                      out_dim=out_dim,
                      **cfg_model['lnn_kwargs'])
    ...
    gnn = DGLGraphNetwork(gnn_kwargs=gnn_kwargs,
                          lnn_kwargs=lnn_kwargs,
                          gnn_architecture=cfg_model['gnn_architecture'])
    gnn = gnn.to(device=device, dtype=dtype)
    ...
    for metric_name, metric_kwargs in selected_metrics.items():
        ...
        if threshold is None:
            ...
            for th in ths:
                ...
                thresholder = Thresholder(threshold=th, operator=operator, th_on_pred=th_on_pred,
...
def config_load_model_wrapper(cfg_reg, metrics, metrics_on_progress_bar, model, layer_name, train_dt, val_dt, device,
                          dtype):
