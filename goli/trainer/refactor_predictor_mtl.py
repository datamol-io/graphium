...
class PredictorModule(pl.LightningModule):
    ...
    def compute_loss(
        ...
        # They all have the same loss function. Get the first wrapped_loss_fun
        #wrapped_loss_fun = next(iter(wrapped_loss_fun_dict.values()))
    ...
    def _general_step(
        ...
        #step_dict[f"{self.loss_fun[self.tasks[0]]._get_name()}/{step_name}"] = loss.detach().cpu()
        step_dict[f"loss/{step_name}"] = loss.detach().cpu()

        return loss, step_dict

    def get_metrics_logs(
        self, preds: Dict[str, Tensor], targets: Dict[str, Tensor], weights: Optional[Tensor], step_name: str, loss: Tensor
    ) -> Dict[str, Any]:
        r"""
        ...
        for task in self.tasks:
            targets_dict[task] = targets[task].to(dtype=preds[task].dtype, device=preds[task].device)
        #targets = targets.to(dtype=preds.dtype, device=preds.device)

        # Compute the metrics always used in regression tasks
        task = self.tasks[0]
        metric_logs = {}
        metric_logs[f"loss/{step_name}"] = nan_mean(loss)           # To get it running
        metric_logs[f"mean_pred/{step_name}"] = nan_mean(preds[task])
        metric_logs[f"std_pred/{step_name}"] = nan_std(preds[task])
        metric_logs[f"median_pred/{step_name}"] = nan_median(preds[task])
        metric_logs[f"mean_target/{step_name}"] = nan_mean(targets_dict[task])
        metric_logs[f"std_target/{step_name}"] = nan_std(targets_dict[task])
        metric_logs[f"median_target/{step_name}"] = nan_median(targets_dict[task])
...
#        metric_logs = {key: metric.detach().cpu() for key, metric in metric_logs.items()}
    ...
    def _general_epoch_end(self, outputs: Dict[str, Any], step_name: str) -> None:
        ...
        """Problem: we need to update the predictor results to make the metrics_logs, but we need the metrics_logs to update the predictor_result"""
